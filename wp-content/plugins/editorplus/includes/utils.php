...
function editorplus_range_convert( $css_property, $range, $css_value = false ) {
	...
	$css_style    = $range['value'] . $range['unit'];
...
function editorplus_convert_extended_list_block( $block, $selector ) {
	...
	if ( $use_icon && isset( $attributes['icon'] ) ) {
		...
		$additional_styles .= sprintf(
			...
			'' !== $attributes['iconBackground']['desktop']['color'] ? sprintf( 'background-color:%1$s;', $attributes['iconBackground']['desktop']['color'] ) : '',
			'' !== $attributes['iconColor']['desktop']['color'] ? sprintf( 'color:%1$s;', $attributes['iconColor']['desktop']['color'] ) : '',
	...
	if ( '' !== trim( $generated_icon_tablet_styling ) ) {
		$generated_styling .= editorplus_generate_raw_css_output_for(
			...
			sprintf(
				'.%1$s .wp-block-button__link:%2$s { %3$s }',
				$selector,
				'right' !== $attributes['epCustomButtonsAlignment']['tablet'] ? 'before' : 'after',
	...
	if ( '' !== trim( $generated_icon_mobile_styling ) ) {
		$generated_styling .= editorplus_generate_raw_css_output_for(
			...
			sprintf(
				'.%1$s .wp-block-button__link:%2$s { %3$s }',
				$selector,
				'right' !== $attributes['epCustomButtonsAlignment']['mobile'] ? 'before' : 'after',
				$generated_icon_mobile_styling
			)
		);
	}

	$generated_icon_hover_styling = editorplus_get_icon_with_styles( 'desktop', $attributes );

	if ( '' !== trim( $generated_icon_hover_styling ) ) {
		$generated_styling .= sprintf(
			'.%1$s .wp-block-button__link:hover:%2$s { %3$s }',
			$selector,
			'right' !== $attributes['epCustomButtonsAlignment']['hover'] ? 'before' : 'after',
...
function editorplus_animation_convert( $block, $selector ) {
	...
	if ( '' !== $attributes['type'] ) {
		...
		return sprintf(
			...
			join( '.', array( $selector, $animation_selector ) )
		);
	}
...
function editorplus_shape_divider_convert( $block, $selector ) {
	...
	if ( 'top' === $position ) {
		...
		$generated_divider_styles .= 'bottom:0!important;left:0!important;';
	}
